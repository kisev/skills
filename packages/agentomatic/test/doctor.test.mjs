...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "agentomatic-doctor-"));
...
test("doctor never serializes config secrets and classifies collisions as problems", async () => {
  ...
  try {
    ...
    assert.match(
      rendered,
      new RegExp(
        `npx --yes ${escapeRegExp(`@kisev/agentomatic@${PACKAGE_VERSION}`)} install --dry-run`,
      ),
    );
    assert.doesNotMatch(rendered, /npm exec -- agentomatic/);
  } finally {
    rmSync(item.root, { recursive: true, force: true });
  }
});
