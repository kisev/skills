...
const packageName = "@kisev/agentomatic";
...
export async function collectDoctorFacts(
  ...
  let manifest = await regular(join(deployment, ".agentomatic-manifest.json"));
  if (manifest.status === "missing")
    manifest = await regular(join(deployment, ".skills-opencode-manifest.json"));
  ...
  checks.push(
    check(
      ...
      {
        path: ".agentomatic-manifest.json",
  ...
  const profileManifest = await regular(
    join(deployment, ".agentomatic", "agent-profiles.manifest.json"),
  );
  ...
  checks.push(
    check(
      ...
      "Agent profile manifest is classified",
      {
        ...
        path: ".agentomatic/agent-profiles.manifest.json",
      },
      ["Run the installer preview for this scope."],
    ),
  );
