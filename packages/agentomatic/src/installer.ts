...
const MANIFEST_NAME = ".agentomatic-manifest.json";
const LEGACY_MANIFEST_NAME = ".skills-opencode-manifest.json";
...
function ownsPackage(name: unknown): name is string {
...
async function currentManifest(
  root: string,
): Promise<{ manifest?: Manifest; raw?: Buffer; manifestPath?: string }> {
  for (const name of [MANIFEST_NAME, LEGACY_MANIFEST_NAME]) {
    const path = destination(root, name);
    const raw = await readRegular(path);
    if (raw) return { manifest: parseManifest(raw, path), raw, manifestPath: path };
  }
  return {};
}
...
async function validateGenericDeployment(
  ...
  if (!owned.manifestPath || ((await lstat(owned.manifestPath)).mode & 0o777) !== 0o600)
    throw new InstallerError(
      "final_validation_failed",
