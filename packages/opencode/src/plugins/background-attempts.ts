...
type Attempt = { schema_version: 1; attempt_id: string; parent_session_id: string; project: string; directory: string; task: string; category: string; agent: string; routing_decision_digest: string; status: Status; revision: number; priority: number; read_only: boolean; created_at: string; receipts: Array<Record<string, unknown>>; child_session_id?: string; workspace_id?: string; result?: { summary: string; evidence: string[] }; error?: { message: string; retryable: boolean } };
...
export async function backgroundAttempts({ client, directory, cwd }: { client: Client; directory?: string; cwd?: string }, options: BackgroundAttemptsOptions = {}) {
  ...
  const attempts = tool({
    ...
    args: { action: tool.schema.enum(["start", "list", "status", "result", "cancel"]), task: tool.schema.string().optional(), category: tool.schema.string().optional(), decision: tool.schema.any().optional(), priority: tool.schema.number().optional(), read_only: tool.schema.boolean().optional(), attempt_id: tool.schema.string().optional(), expected_revision: tool.schema.number().optional(), expected_status: tool.schema.enum(["queued", "running", "waiting"]).optional() },
    async execute(args: { action: "start" | "list" | "status" | "result" | "cancel"; task?: string; category?: string; decision?: unknown; priority?: number; read_only?: boolean; attempt_id?: string; expected_revision?: number; expected_status?: "queued" | "running" | "waiting" }, context: { sessionID: string }) {
