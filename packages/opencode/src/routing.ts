...
export type RoutingInput = {
  ...
  budget?: { cost_class?: string; latency_class?: string };
};
export type RoutingDecision = {
  ...
  agent?: string;
  reason_codes: string[];
  alternatives: { agent: string; excluded_reasons: string[] }[];
...
export function resolveRouting(input: RoutingInput): RoutingDecision {
  ...
  const dynamicCritics =
    input.category === "review"
      ...
      : [];
  const profiles =
    input.trusted_override && input.override && !category.profiles.includes(input.override as never)
  ...
  const selected = input.override
    ...
    : eligible[0];
  const card =
    input.execution_card === undefined ? undefined : validateExecutionCard(input.execution_card);
...
export function validateExecutionCard(
  card: unknown,
): { valid: true; card: ExecutionCard } | { valid: false; failedField: string } {
  ...
  for (const operation of c.operations) {
    ...
    if (item.path !== undefined && (typeof item.path !== "string" || !item.path))
      return { valid: false, failedField: "operations" };
    if (
      item.confirmation_ref !== undefined &&
      (typeof item.confirmation_ref !== "string" || !item.confirmation_ref)
    )
      return { valid: false, failedField: "operations" };
    if (
      Object.keys(item).some(
        (key) => !["kind", "command", "path", "confirmation_ref"].includes(key),
      )
    )
      return { valid: false, failedField: "operations" };
  ...
  if (
    Object.keys(confirmations).some(
      (key) => !["execution", "publication", "history_rewrite"].includes(key),
    )
  )
    return { valid: false, failedField: "confirmations" };
  ...
  if (!writeSet.every((path) => stepPaths.includes(path)))
    return { valid: false, failedField: "steps" };
  const operations = c.operations as Array<{
    kind: string;
    path?: string;
    confirmation_ref?: string;
  }>;
  if (
    !writeSet.every(
      (path) =>
        operations.some((operation) => operation.kind === "write" && operation.path === path) &&
  ...
  for (const operation of operations) {
    if (
      ["commit", "rebase", "push", "merge", "tag", "release"].includes(operation.kind) &&
    ...
    if (operation.kind === "rebase" && operation.confirmation_ref !== confirmations.history_rewrite)
      return { valid: false, failedField: "confirmations" };
    if (
      ["push", "merge", "tag", "release"].includes(operation.kind) &&
...
export class ExecutionCardLifecycle {
  ...
  transition(
    ...
    identity: { card_id: string; revision: number },
  ): ExecutionCardStatus {
...
export class RoutingGate {
  ...
  grant(sessionID: string, decision: RoutingDecision, context?: ReceiptContext): RoutingReceipt {
    ...
    const cardDigest =
      context?.card === undefined ? decision.execution_card_digest : digest(context.card);
    const receiptBase = {
      schema_version: 1 as const,
