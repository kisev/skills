...
- Keep the portable version manifest, OpenCode package and lockfile, catalog, changelog heading, annotated `vX.Y.Z` tag, Pages distribution, npm artifact, and GitHub Release on one commit; after confirmation, push that commit to `release/vX.Y.Z` and require the clean `release-preflight.yml` gate, then atomically push the same commit to `main` with its tag and delete the temporary branch; `.github/workflows/publish.yml` owns published-tag validation, publication, verification, and release creation.
