...
- Keep the portable version manifest, OpenCode package and lockfile, changelog heading, annotated `vX.Y.Z` tag, Pages distribution, npm artifact, and GitHub Release aligned. Prepare the user-selected stable version on `dev`, merge `dev` into `main`, ensure no publication run is active or pending, then require explicit confirmation before tagging and pushing the resulting `main` commit; `.github/workflows/publish.yml` owns stable and dev publication.
