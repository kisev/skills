...
`task-triage` дополнительно получает специализированную среду выполнения для
GitLab. Она хранит адресуемые по содержимому свидетельства и анализ в XDG state,
повторно использует анализ неизменившихся задач и атомарно обновляет стабильные
Markdown-отчёты, не изменяя GitLab.
Все три task skills получают единый валидатор release planning. Triage владеет
свидетельствами о релизах и milestone, review проверяет общий sidecar, а prepare
использует актуальный scoped triage вместо копирования SemVer-политики.
...
`skills`. Необязательный пакет npm `@kisev/agentomatic` предоставляет
`agentomatic` для интеграции с OpenCode. Он не устанавливает и не обновляет
переносимые навыки, не проверяет и не удаляет их; их жизненным циклом управляет
отдельный CLI `skills`.
...
После push в `dev` тот же конвейер публикации выполняет полный gate без
стабильного тега. Затем он создаёт техническую prerelease-версию, заменяет только
Pages `/dev` и перемещает только npm dist-tag `dev`. Каждый деплой Pages объединяет
стабильный корень и `/dev`, поэтому обновление одного канала сохраняет другой.

## Интеграция с хостами

Переносимые навыки не зависят от конкретного хоста, а каждый опубликованный архив
самодостаточен. `packages/agentomatic/` является необязательным адаптером OpenCode и
...
`packages/agentomatic/dist/assets/`.
...
используют тот же project default. Очистка переносимых навыков принадлежит внешнему
`skills` CLI; пакет не создаёт для неё snapshots и не обеспечивает rollback.
Глобальный архив общий, а архивы проектов изолированы
...
`critic`. Менеджер делегирует всю содержательную работу, включая быстрые read-only
запросы, а изменения документации направляет как `implementation`.
Агент `review` доступен напрямую и как подчинённый manager. Он сам проводит основное
ревью и вызывает критиков по правилам выбранного скилла. Изменение исходников
допускается только по явному запросу пользователя, в отдельной фазе исправлений.
...
- Dev-публикация выполняет полный gate для push в `dev`, проверяет точные артефакты
  Pages и npm и не может обновить стабильный Pages или npm `latest`.
