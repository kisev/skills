...
`skills@latest` с теми же агентами и областью, что и при установке. Новые
публикуемые навыки содержат `metadata.source: "https://kisev.github.io/skills"`.
`reconcile` может архивировать помеченное устаревшее имя, напрямую вызвать точную
версию `skills` для OpenCode и Codex и откатить пути из preview при ошибке;
установки без маркера по-прежнему очищаются вручную.
...
точно подтверждённой принадлежностью, прежде чем удалить развёрнутые копии. Для
помеченного переносимого навыка source-маркер подтверждает право очистки, а его
текущие байты архивируются перед транзакционным удалением. Пользовательские,
неизвестные, небезопасные и неоднозначные элементы остаются без изменений и
отображаются как `findings` или `conflicts`. Рабочие деревья и runtime state
сохраняются. Архив поддерживает транзакционный откат и доступен через `doctor`;
команд `restore` и `purge` нет.
