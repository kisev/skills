# Architecture Decisions

This append-only index preserves current and superseded decisions about the
target architecture. It contains neither proposals for delivery nor roadmap
items.

- [ADR-0001: Direct Git self-contained skills](0001-direct-git-self-contained-skills.md) - superseded
- [ADR-0002: Target public surface](0002-target-public-surface.md) - superseded
- [ADR-0003: Portable coordinator and routing adapter](0003-portable-coordinator-routing-adapter.md) - superseded
- [ADR-0004: Content-addressed archive](0004-content-addressed-archive.md) - accepted
- [ADR-0005: GitHub Pages well-known distribution](0005-github-pages-well-known-distribution.md) - accepted
- [ADR-0006: Retire generic doit coordinator](0006-retire-generic-doit-coordinator.md) - superseded
- [ADR-0007: Make administration CLI-only](0007-make-administration-cli-only.md) - accepted
- [ADR-0008: Compose persistent GitLab task triage](0008-compose-persistent-gitlab-task-triage.md) - accepted
- [ADR-0009: Share task release planning](0009-share-task-release-planning.md) - accepted
- [ADR-0010: Separate review orchestration and guarded publication](0010-separate-review-and-publication.md) - accepted
