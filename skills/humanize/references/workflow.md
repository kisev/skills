...
2. Preserve the language, project style, and ongoing conversation style unless they conflict with an explicit request.
...
- Use the ordinary hyphen `-` and straight double quotes `"` in newly written Russian text.
- Do not restate the conclusion in other words at the end or split a simple thought into unnecessary headings or lists.
- A reply in an existing thread continues the conversation: answer the question or reaction directly first, then add only a new check or correction.
- Do not replace the author's role with the reviewer's role or attribute actions, decisions, or promises to the user.
...
- Use plain, precise Russian while retaining the project's familiar technical language. Do not add artificial jargon.
- Do not change exact quotations, code, command output, paths, APIs, file names, or external names solely for style.

## Reviews and comments

- One comment normally fits in two or three sentences.
- In a GitLab MR context, write "thread" in Russian as "thread"; do not rename exact API fields or commands.
