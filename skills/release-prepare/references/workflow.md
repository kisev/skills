...
Read `references/interaction-contract.md`, `references/gitlab-workflow.md`, `references/portable-gitlab-contracts-v2.md`, `references/language-policy.md`, and `references/release-examples.md`. Accept exactly one release MR URL; do not infer it from a branch and do not create a batch.
...
Put exactly `title`, `description`, `version`, `announcement`, and `illustration_prompt` in the scaffold content JSON. Create the plan with `scripts/prepare_release.py scaffold --bundle <evidence-path> --inventory <inventory-path> --content <content-path>`. It writes an immutable JSON envelope, a Markdown plan, and separate immutable release description, announcement, and illustration-prompt companions. It does not provide mutation commands.
...
Immediately before manual publication, run `scripts/prepare_release.py finalize --plan <publication-plan-path>`. Changed exact base/start/head SHA, MR object, labels, discussions, diff, commits, pipelines, release boundary, commit associations, inventory completeness, or any modified artifact blocks readiness. Do not create, update, approve, merge, push, publish a GitLab Release, choose a Mattermost channel, or send the announcement.
