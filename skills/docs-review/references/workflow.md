...
The input is a path or scope. Without one, establish the boundary through the host interaction mechanism or chat. If it is in `specs/`, stop and offer `project-spec` in `spec-audit` mode. Do not change the repository, documents, external systems, or create artifacts; report findings only in chat and never publish them.
...
- If the area is in `specs/`, stop and propose `project-spec` in `spec-audit` mode.
